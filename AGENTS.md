...
- Before editing or generating project files, run `git branch --show-current` and confirm that this is the canonical application on `main`. Read the current application version from `apps/desktop/package.json`; that file is the version source of truth.
- If the worktree or branch does not match the canonical application, stop and ask the user before moving changes.
